...
class My_4layer_DNN(nn.Module):
    ...
    def forward(self, M_antenna_num, N_grid_point_num, tau, noiseSTD_per_dim, A_BS,
        ...
        for t in range(tau):
            ...
            phi_idx_est_dict.append(torch.argmax(posteriors, dim=1))  # 将得到的 AOA索引信息 添加到 列表 中
        ...
        return phi_idx_est, loss, phi_idx_est_dict
...
for epoch in range(n_epochs):
    ...
    for rnd_indices in range(batch_num_per_epoch):   #  batch 级别需要 运行batch_per_epoch， 也就是说 有 batch_per_epoch 个 batch
        ...
        phi_idx_est, loss, phi_idx_est_dict = my_4layer_dnn(M_antenna_num=M_antanna_num,
    ...
    # if epoch % 1 == 0:
    if epoch % 10 == 9:
        phi_idx_est_val, loss_val, phi_idx_est_dict_val = my_4layer_dnn(M_antenna_num=M_antanna_num,
                                                            N_grid_point_num=N_grid_point_num,
                                                            tau=tau,
                                                            noiseSTD_per_dim=noiseSTD_per_dim,
                                                            A_BS=A_BS,
...
with torch.no_grad():  # 在评估模式下不需要计算梯度
    for j in range(scale_factor):
        ...
        for i in range(len(snrdBvec)):
            phi_idx_est_test, loss_test, phi_idx_est_dict_test = my_4layer_dnn(M_antenna_num=M_antanna_num,
                                                                               N_grid_point_num = N_grid_point_num,
                                                                               tau = tau,
                                                                               noiseSTD_per_dim = noiseSTD_per_dim,
                                                                               A_BS = A_BS,
