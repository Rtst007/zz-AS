    ...
    def forward(self, alpha_batch, phi_batch, P_temp, args, criterion):
        ...
        for id in range(tnum):
            ...
            for t in range(tau):  # 表示 RNN 网络 有 tau 个 LSTM cell
                ...
                # if t > 0:
                #     phi_batch = phi_batch + 1*(np.pi/180) * torch.ones((batch_size, 1), dtype=torch.float32, device=device)
                ...
                lammbdaa = 3.0*10**8/(30.0*10**9)  # lammbdaa = c/fc,  fc = 30 GHz  mmWave band
                sigma_RCS = 1.0  # 人的雷达截面积 m^2
                d_len = 10.0

                # beta_path_loss = np.sqrt(   (lammbdaa**2) * sigma_RCS / ( (4.0*np.pi)**3 * (d_len**4) )   )

                beta_path_loss = 1.0

                y_complex = (torch.sqrt(P_temp) + 1j * 0.0) * beta_path_loss * y_noiseless + noise_complex
