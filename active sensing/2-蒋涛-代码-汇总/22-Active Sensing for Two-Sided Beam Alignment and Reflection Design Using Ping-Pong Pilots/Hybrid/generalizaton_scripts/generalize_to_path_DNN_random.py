...
model_path = './params/DNN_v3_L_SNR_tau1_tau2_N1_N2_' + str((L_train, snrdB, tau1, tau2, N1, N2))
######################################################
...
with tf.name_scope("channel_sensing"):
    V = np.random.normal(loc=0, scale=1, size=[1, N1, tau1]) \
        + 1j * np.random.normal(loc=0, scale=1, size=[1, N1, tau1])
    V = V / np.linalg.norm(V, axis=1, keepdims=True)
    W = np.random.normal(loc=0, scale=1, size=[1, N2, tau2]) \
        + 1j * np.random.normal(loc=0, scale=1, size=[1, N2, tau2])
    W = W / np.linalg.norm(W, axis=1, keepdims=True)

    V_tf = tf.get_variable('V_tf', initializer=V.astype(np.complex64), trainable=False)
    W_tf = tf.get_variable('W_tf', initializer=W.astype(np.complex64), trainable=False)
...
with tf.Session() as sess:
    ...
    sio.savemat('./results/DNN_random_tau_generalize' + str((tau1, tau2)) +
                str(L)+
                '.mat',
