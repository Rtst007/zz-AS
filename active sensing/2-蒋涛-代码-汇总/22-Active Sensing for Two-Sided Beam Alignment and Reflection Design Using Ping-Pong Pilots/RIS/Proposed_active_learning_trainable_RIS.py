...
batch_size_val = 2000
model_path = './params/RNN_both_control_trainable_RIS_' + str((L1, L2, snrdB, tau, M1, M2))
######################################################
...
with tf.name_scope("channel_sensing"):
    ...
    for t in range(tau):
        ...
        v_uplink_real = tf.get_variable("v_rx_real" + str(t), shape=(1, N1 * N2), trainable=True)
        v_uplink_imag = tf.get_variable("v_rx_imag" + str(t), shape=(1, N1 * N2), trainable=True)
        v_complex = tf.complex(v_uplink_real, v_uplink_imag)
        v = v_complex / tf.cast(tf.abs(v_complex), tf.complex64)
        ...
        v_uplink_real = tf.get_variable("v_tx_real" + str(t), shape=(1, N1 * N2), trainable=True)
        v_uplink_imag = tf.get_variable("v_tx_imag" + str(t), shape=(1, N1 * N2), trainable=True)
        v_complex = tf.complex(v_uplink_real, v_uplink_imag)
        v = v_complex / tf.cast(tf.abs(v_complex), tf.complex64)
...
with tf.Session() as sess:
    ...
    #
    # sio.savemat('./results/RNN_both_control_final_RIS_from_both_trainable_RIS_' + str(tau) + '.mat',
