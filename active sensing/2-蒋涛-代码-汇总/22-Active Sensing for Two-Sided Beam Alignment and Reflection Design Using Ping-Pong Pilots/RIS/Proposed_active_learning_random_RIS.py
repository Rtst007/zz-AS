...
'''
Tx and Rx control RIS.
Beamforming vectors are actively updated.
Reflection coefficients are trainalbe parameters.
The RIS is updated based on the state information from both side.
'''
...
batch_size_val = 2000
model_path = './params/RNN_random_RIS_' + str((L1, L2, snrdB, tau, M1, M2))
v1_np = np.random.normal(loc=0,scale=1, size=[tau, N]) + 1j * np.random.normal(loc=0, scale=1, size=[tau, N])
v2_np = np.random.normal(loc=0,scale=1, size=[tau, N]) + 1j * np.random.normal(loc=0, scale=1, size=[tau, N])
######################################################
...
with tf.name_scope("channel_sensing"):
    ...
    for t in range(tau):
        ...
        'Trainable RIS sensing vectors for transmission from Tx to Rx'
        v_np = np.reshape(v2_np[t, :], [1, N])
        v_complex = tf.get_variable('v2_complex' + str(t), initializer=v_np.astype(np.complex64), trainable=False)
        v = v_complex / tf.cast(tf.abs(v_complex), tf.complex64)

        'Update effective channel'
        G = A @ tf.linalg.diag(v) @ B
        if t==0:  bf_gain_rnd = tf.reduce_mean(tf.abs(tf.transpose(tf.conj(w_a2), perm=[0, 2, 1]) @ G @ w_b1) ** 2)
        ...
        'Trainable RIS sensing vectors for transmission from Rx to Tx'
        v_np = np.reshape(v1_np[t, :], [1, N])
        v_complex = tf.get_variable('v1_complex' + str(t), initializer=v_np.astype(np.complex64), trainable=False)
        v = v_complex / tf.cast(tf.abs(v_complex), tf.complex64)
...
with tf.Session() as sess:
    ...
    # sio.savemat('./results/RNN_random_RIS_' + str(tau) + '.mat',
